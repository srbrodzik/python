...
#    print lon[:12]
    ...
    cs = np.full(cs_dbz.shape, np.nan)
    cc = np.zeros_like(cs) - 1
    bkgnd = np.full(cs_dbz.shape, np.nan)


    # if dbz is a masked array, then just grab the opposite of the mask
    if isinstance(cs_dbz, np.ma.masked_array):
	bad = deepcopy(cs_dbz.mask)
    else:
	bad = cs_dbz == np.nan
...
#    bkgnd[bad] = -10.0
    ...
    inThr = (bkgnd < 0.) & ((cs_dbz - bkgnd) > bg_diff)
    cc[inThr] = 1

    #; Next line uses SYH 1995 radius algorithm
    if method == 'SYH':
	inCon = (bkgnd >= 0.) & (bkgnd < tune_thresh) & (cs_dbz-bkgnd > (bg_diff-(bkgnd**2.)/180.))
#; This line uses YH (1998) climatological tuning algorithm
    else:
	inCon = (bkgnd >= 0.) & (bkgnd < tune_thresh) & (cs_dbz-bkgnd > a*np.cos((np.pi*bkgnd)/(2.*b)))
