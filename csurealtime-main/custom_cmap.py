...
class CreateColormap(object):
    def __init__(self, colors, position=None, bit=False):
        '''
    ...
    def __call__(self, reverse=False):
        ...
        if self.position == None:
            self.position = np.linspace(0, 1, colors.size)
        else:
            if self.position.size != colors.size:
                sys.exit("position length must be the same as colors")
            elif self.position[0] != 0 or self.position[-1] != 1:
        ...
        for pos, color in zip(self.position, colors):
...
my_cmap = CreateColormap(colors, bit=True)
print my_cmap
### Use your colormap
...
### take out a step.
plt.pcolor(np.random.rand(25,50), cmap=CreateColormap(colors))
plt.colorbar()
...
position = [0, 0.3, 1]
plt.pcolor(np.random.rand(25,50), cmap=CreateColormap(colors, position=position))
plt.colorbar()
