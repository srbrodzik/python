...
# Copyright (c) 2013 Thomas Chubb 
    ...
    #--------------------------------------------------------------------
        ...
        elif self.fmt == 'SMN': # THIS IS FOR READING IN RELAMPAGO SMN COMMA SEPARATED DATA FILE FORMATS, FOR SPECIAL FIELD OPS SOUNDINGS
            prefix = os.path.splitext(os.path.basename(fname))[0]
            time_string = prefix.split('_')[0]+prefix.split('_')[1]+'0000'
            self.sounding_date = datetime.strptime(time_string, '%y%m%d%H%M%S')

            station = os.path.basename(fname).split('_')[2]
            self.station = station.split('.')[0]
            
            htcol = 1; prescol = 0; tempcol = 2; dewcol = 3; spdcol = 5; drctcol = 4

            file_data = np.genfromtxt(fname, delimiter = ',')
