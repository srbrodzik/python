...
#EDITED: S. Brodzik, UW, FEB 2022
# added Purdue and VALtxt formats

#EDITED: S. Brodzik, UW, May 2022
# added definition of 'CWB' format
...
# Copyright (c) 2013 Thomas Chubb
    ...
    def readfile(self, fname):
        ...
        # Added by S Brodzik (May 2022)
        elif self.fmt == 'CWBshr': #used for CWB 'shr.txt' soundings

            # read in file after header as DataFrame
            df = pd.read_csv(fname, header=None)

            df.columns = ['time (hh:mm)','pres (hPa)','ht (m)','temp (degC)','U (m/s)','dew_pt (degC)',
                          'wdir (deg)','wspd (m/s)']
        ...
        # Added by S Brodzik (May 2022)
        elif self.fmt == 'CWBfix': #used for CWB 'fix.txt' soundings

            # read in file after header as DataFrame
            df = pd.read_csv(fname,skiprows=2,header=None)

            df.columns = ['Time','Height','P','T','U','Td','WS','WD']
        ...
        # Added by S Brodzik (May 2022)
        elif self.fmt == 'CWBedt': #used for CWB 'edt.txt' soundings

            # read in file after header as DataFrame
            #df = pd.read_csv(fname,skiprows=2,delim_whitespace=True)
            df = pd.read_csv(fname,skiprows=3,header=None)

            df.columns = ['Time','Height','P','T','U','WS','WD','Ascent']
        ...
        # Added by S Brodzik (Feb 2020)
            ...
            # read in file after header as DataFrame
            df = pd.read_csv(fname, skiprows=7, header=None)
        ...
        elif self.fmt == 'VALtxt': #used for Valparaiso sondes

            # read in file after header as DataFrame and sort into descending pressures
            # need to so some playing around due to using two sets of significant levels
