...
########################## dealias full volume
    ...
    print "Doing dealias using region with 4DD based as input"
    dealias_data.append(pyart.correct.dealias_region_based(radar, refl_field = 'AZ', vel_field = 'VC', corr_vel_field = 'VE', keep_original = True))
########################## delias error
#    print "Full Volume Error in dealias_fourdd has occurred. "

##########################  Full volume save and plot
    print "save data"
    radar.add_field('VE', dealias_data[1])
    radar.fields['VE']['data'].mask = np.ma.mask_or(combine,radar.fields['VE']['data'].mask)
    pyart.io.write_cfradial(fname[0:-3]+'.unfold.nc',radar,format = 'NETCDF4',time_reference = False)
