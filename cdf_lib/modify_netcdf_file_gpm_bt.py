...
# read vars
to = ncid.variables['time_offset'][:]
ntimes = to.size
lats = ncid.variables['latitude'][:]
nlats = lats.size
lons = ncid.variables['longitude'][:]
nlons = lons.size
alts = ncid.variables['altitude'][:]
nalts = alts.size
nalts_125 = 125
...
# create vars
bt_var = ncid.createVariable('base_time',np.int32,zlib=True )
to_var = ncid.createVariable('time_offset',np.float64,('time'),zlib=True )
...
# define attributes
bt_var.units = 'seconds since 1970-01-01T00:00:00Z'
to_var.units = 'seconds since base_time'
