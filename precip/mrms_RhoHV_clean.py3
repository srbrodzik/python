...
baseDir = '/home/disk/monsoon/precip/raw/radar/2021/mrms/MergedRhoHV'
...
for date in os.listdir(baseDir):
    if date.startswith('20210') and os.path.isdir(baseDir+'/'+date):
        ...
        os.mkdir('10')
        flist = glob.glob('MRMS_MergedRhoHV_19.00_20210???-???5??.*')
        print('len(flist) =',len(flist))
        if len(flist) >= 144:
            flistAll = glob.glob('MRMS_MergedRhoHV_*_20210???-???5??.*')   
