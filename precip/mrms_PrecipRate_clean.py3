...
baseDir = '/home/disk/monsoon/precip/raw/radar/2021/mrms/PrecipRate'

for date in os.listdir(baseDir):
    if date.startswith('2021083') and os.path.isdir(baseDir+'/'+date):
        print('date =',date)
        os.chdir(baseDir+'/'+date)
        os.mkdir('10')
        flist = glob.glob('MRMS_PrecipRate_*_20210???-???6??.*')
        for file in flist:
