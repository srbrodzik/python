...
def conv_strat_latlon(dbz, missing_value, lat2d, lon2d, CoreThresh=42.0, method='SYH', a=8, b=64, tune_thresh=42.0, sm_rad=15, fill_dbz=25.0, bg_diff=10):

#    xdim = N_ELEMENTS(x[*,0])-1
#    ydim = N_ELEMENTS(y[0,*])-1

    print >>sys.stdout, 'dbz[0:10,0] = ', dbz[0:10,0]
    print >>sys.stdout, 'lat2d[0:10] = ', lat2d[0:10]
    print >>sys.stdout, 'lon2d[0:10] = ', lon2d[0:10]
    print >>sys.stdout, 'CoreThresh  = ', CoreThresh
    print >>sys.stdout, 'method      = ', method
    print >>sys.stdout, 'a           = ', a
    print >>sys.stdout, 'b           = ', b
    print >>sys.stdout, 'tune_thresh = ', tune_thresh
    print >>sys.stdout, 'sm_rad      = ', sm_rad
    print >>sys.stdout, 'fill_dbz    = ', fill_dbz
    print >>sys.stdout, 'bg_diff     = ', bg_diff
	...
	#bad = dbz == np.nan
	bad = dbz == missing_value  #SRB

    good = np.logical_not(bad)
	
    #zlin[bad] = np.nan
    #dbz[bad] = np.nan
    dbz[bad] = fill_dbz  # SRB why is 25 the default -- kind of high?
    #zlin[bad] = 0.0

    # calculate linear reflectivity
    zlin = 10.**(dbz/10.)
    #zlin = 10.**(dbz[good]/10.)    #SRB
