...
def writeCFRELnetcdf(ncname,types,deepcoszero,shallowconvmin,minZdiff,truncZconvthres,
                     dBZformaxconvradius,weakechothres,backgrndradius,maxConvRadius,
                     minsize,startslope,maxsize,title,institution,source,references1,
                     references2,comment,timeVal,xVal,yVal,gmVal,
                     lat_origin,lon_origin,raintype,missing_value,author):
    ...
    ncid.comment = comment
    if author == 'MS':
        ncid.comment2 = 'Powell algorithm with mods to outputs to reflect Steiner: UNCERTAIN and ISO_CONV_CORE changed to CONVECTIVE; ISO_CONV_FRINGE changed to STRATIFORM; WEAK_ECHO changed to OTHER'
    ncid.history = 'File created ' + currentTime
