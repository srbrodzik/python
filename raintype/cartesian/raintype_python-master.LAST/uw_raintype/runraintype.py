...
fileDir = '/home/disk/mjo/dynamo/data.server/interp/QCed/spolka/sur_1km_cf/refl/20111025'
fileDirOut = '/home/disk/mjo/dynamo/data.server/interp/QCed/spolka/rain_type_sur_test/20111025'
    ...
    if rtout is not None:
      if outputFormat == 'zeb':
        net.writeZebNetcdf(ncname,types,deepcoszero,shallowconvmin,minZdiff,truncZconvthres,
                           ...
                           references2,comment,bt,toff,lat,lon,alt,
                           dx,dy,dz,rtout,missing_value)
      elif outputFormat == 'cf':
        net.writeCFnetcdf(ncname,types,deepcoszero,shallowconvmin,minZdiff,truncZconvthres,
                          dBZformaxconvradius,weakechothres,backgrndradius,maxConvRadius,
                          minsize,startslope,maxsize,title,institution,source,references1,
                          references2,comment,tim,x,y,lat,lon,gm,
                          lat_origin,lon_origin,rtout,missing_value)
      else:
        net.writeBasicNetcdf(ncname,types,deepcoszero,shallowconvmin,minZdiff,truncZconvthres,
                             dBZformaxconvradius,weakechothres,backgrndradius,maxConvRadius,
                             minsize,startslope,maxsize,title,institution,source,references1,
                             references2,comment,dx,radar_lat,radar_lon,raintype.shape[0],rtout.shape[1],
                             rtout,missing_value)
