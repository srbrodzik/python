...
def convectivecore(background,
  ...
  #Compare refl to background - DO WE NEED TO CHECK refl & background FOR MISSING VALS?
  #Compute zDiff
  zDiff = minZdiff * (np.cos((np.pi)*background*0.5/deepcoszero))
  zDiff[(background < 0)] = minZdiff
  zDiff[zDiff < 0] = 0  # in original code but does this make sense?
  ...
  convsfmat[refl <= mindBZuse] = types['NO_SFC_ECHO']
  convsfmat[np.logical_and(refl > mindBZuse,refl < weakechothres)] = types['WEAK_ECHO']
  convsfmat[np.logical_and(convsfmat != types['CONVECTIVE'],
                           np.logical_and(convsfmat != types['WEAK_ECHO'],
                                          convsfmat != types['NO_SFC_ECHO']))] = types['STRATIFORM']
  ...
  #If there is no data, classify as NO_SFC_ECHO.
  convsfmat[np.isnan(refl)==1] = types['NO_SFC_ECHO']
  
  #Classify WEAK_ECHO - did this above
