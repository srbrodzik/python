...
def writeBasicNetcdf(ncname,types,deepcoszero,shallowconvmin,minZdiff,truncZconvthres,
    ...
    # THIS WILL WORK AND CAN BE INPLEMENTED AT ANY TIME
...
def writeCFRELnetcdf(ncname,types,deepcoszero,minZdiff,truncZconvthres,
                     dBZformaxconvradius,weakechothres,backgrndradius,maxConvRadius,
                     minradius,mindBZuse,title,institution,source,references1,
                     references2,comment,timeVal,xVal,yVal,gmVal,
                     lat_origin,lon_origin,raintype,missing_value):
    ...
    zcVar = ncid.createVariable('rt_Z_conv',np.float32)
    zwVar = ncid.createVariable('rt_Z_weak',np.float32)
    rmVar = ncid.createVariable('rt_r_min',np.float32)
    zmVar = ncid.createVariable('rt_Z_min',np.float32)
    rt = ncid.createVariable('rain_type',np.int32,('time','y','x'),zlib=True,fill_value=missing_value )
    ...
    # R_min
    rmVar.units = 'km'
    rmVar.long_name = 'min_radius'
    rmVar.comment = ''

    # Z_min
    zmVar.units = 'dBZ'
    zmVar.long_name = 'min_dbz_use'
    zmVar.comment = ''
    ...
    #                           types['WEAK_ECHO']))
    #rt.flag_meanings = "NO_ECHO STRATIFORM CONVECTIVE UNCERTAIN ISO_CONV_CORE ISO_CONV_FRINGE WEAK_ECHO"
    rt.NO_ECHO = types['NO_SFC_ECHO']
    rt.STRATIFORM = types['STRATIFORM']
    rt.CONVECTIVE = types['CONVECTIVE']
    rt.OTHER = types['WEAK_ECHO']
    rt.ancillary_variables = 'rt_Z_th rt_R_bg rt_a rt_b rt_R_conv rt_Z_conv rt_Z_weak rt_r_min rt_Z_min'
