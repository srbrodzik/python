...
for date in dates:
    ...
    for fname in os.listdir(fileDir+'/'+date):
      if fname.endswith('nc'):
        ...
        rt_powell_mask=rt_powell[rt_powell==missing_value_refl]

        if not outputBestOnly:

            #------------------
            #CREATE MIN RAINMAP - REMOVE ECHO TYPE 6 (WEAK_ECHO) FROM ALL CALCS *****************************
            #------------------
            # Create new conv/str/mixed-only rt map (stra=1,3,5, conv=2,4,6) - rt returns with missing values = missing_value_refl (-9999)
            (rt_min,rt_min_mask) = rrf.assign_conv_stra_mixed(rt_powell, types['STRATIFORM'], missing_value_refl)
    
            # Run CSU blended rain algorithm
            (rr_min,method_min) = cbrt.calc_blended_rain_tropical(refl, zdr, kdp, rt_min, ice_flag=False,
        ...
        #CREATE BEST RAINMAP
        #-------------------
        # Create new conv/str/mixed-only rt map (stra=1,5, conv=2,4,6, mixed=3) - rt returns with missing values = missing_value_refl (-9999)
        (rt_best,rt_best_mask) = rrf.assign_conv_stra_mixed(rt_powell, types['UNCERTAIN'], missing_value_refl)
        ...
        if not outputBestOnly:
            ...
            #CREATE MAX RAINMAP
            #------------------
            # Create new conv/str/mixed-only rt map (stra=1,5, conv=2,3,4,6) - rt returns with missing values = missing_value_refl (-9999)
            (rt_max,rt_max_mask) = rrf.assign_conv_stra_mixed(rt_powell, types['CONVECTIVE'], missing_value_refl)

            # Run CSU blended rain algorithm, then apply convective ZR to mixed type
            (rr_max,method_max) = cbrt.calc_blended_rain_tropical(refl, zdr, kdp, rt_max, ice_flag=False,
                                                                  fhc=None, predef='True', band='S',
                                                                  thresh_dz=38.0, thresh_zdr=0.25, thresh_kdp=0.3,
                                                                  thresh_frac_ice=0.1, thresh_nexrad=53.0,
                                                                  r_z_a=216., r_z_b=1.39,
        ...
        rr_best[rt_best_mask==1] = missing_value_refl
        rr_max[rt_max_mask==1] = missing_value_refl
