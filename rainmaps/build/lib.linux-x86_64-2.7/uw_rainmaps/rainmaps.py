...
def rainmaps(refl, zdr, kdp, rt_powell, types, zr_method, sigOverR, uncert_vars, uncert_vals, 
   ...
   if not outputBestOnly:
      ...
      #CREATE MIN RAINMAP
      #------------------
      # Create new conv/str/mixed-only rt map (stra=1,3,5, conv=2,4, noecho=0,6)
   ...
   #CREATE BEST RAINMAP
   #-------------------
   # Create new conv/str/mixed-only rt map (stra=1,5, conv=2,4, mixed=3, noecho=0,6) - rt returns with missing values = missing_value (-9999)
   ...
   if not outputBestOnly:
      ...
      #CREATE MAX RAINMAP
      #------------------
      # Create new conv/str/mixed-only rt map (stra=1,5, conv=2,3,4, noecho=0,6) - rt returns with missing values = missing_value (-9999)
   ...
   if not outputBestOnly:
      ...
      rr_min = rr_best
      rr_max = rr_best
      method_min = method_best
      method_max = method_best
