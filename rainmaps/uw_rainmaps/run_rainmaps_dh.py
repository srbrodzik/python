...
refl_dx = 0.5;                #Grid spacing of Cartesian reflectivity data.  Only used if not in input file
...
## Z-R coefficients and exponents for rain estimation
a_conv = 126.0
b_conv = 1.46
a_stra = 291.0
b_stra = 1.55
...
for date in dates:
    ...
    for fname in os.listdir(fileDir+'/'+date):
      if fname.endswith('nc'):
            ...
            for i in np.arange(1,len(zr_method)+1):
                ...
                for x in range(0,rr_min.shape[0]):
                    for y in range(0,rr_min.shape[1]):
                        if (method_min[x,y] == i) and (rr_min[x,y] <= rmse[i]):
                            zero_mask[x,y] = 1
        ...
        (rt_best,rt_mask) = rrf.assign_conv_stra_mixed(rt_best_all, types['UNCERTAIN'], missing_value_refl)
        ...
        if not outputBestOnly:
            ...
            if rt_best is not None:
                if outputFormat == 'zeb':
                    io.writeZebNetcdfBestOnly(outFile,rt_best_all,rr_best,method_best,
                                              deepcoszero,shallowconvmin,minZdiff,truncZconvthres,dBZformaxconvradius,
                                              weakechothres,backgrndradius,maxConvRadius,minsize,startslope,maxsize,
