...
for file in glob.glob('infoRainNSR_Conv_EchoCores_*_'+iyear+'_*.zip'):
    ...
    for feature in features.keys():
        ...
        crr_var_id.long_name = features[feature]+' convective core rainrate sum based on TRMM PR /FS/SLV/precipRateNearSurface'
        ...
        cct_var_id.long_name = 'number of '+features[feature]+' convective core pixels with TRMM PR /FS/SLV/precipRateNearSurface greater than zero'
                
        srr_var_id = id.createVariable('conv_storm_rain_rate_sum','f4',('time','latitude','longitude',),
                                       fill_value=missing_flt,zlib=True,complevel=5)
        srr_var_id.units = 'mm/hr'
        srr_var_id.long_name = 'storm rainrate sum based on TRMM PR /FS/SLV/precipRateNearSurface'

        sct_var_id = id.createVariable('conv_storm_rain_rate_count','i4',('time','latitude','longitude',),
                                       fill_value=missing_int,zlib=True,complevel=5)
        sct_var_id.units = 'none'
        sct_var_id.long_name = 'number of storm pixels with TRMM PR /FS/SLV/precipRateNearSurface greater than zero'

        if verbose:
            print('Done with vars')
            
        # define global variables
        id.Title = features[feature]+' convective monthly rain rate sums based on TRMM PR /FS/SLV/precipRateNearSurface and /FS/CSF/typePrecip'
        if thresLevel == 'str':
