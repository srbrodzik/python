...
def convert_bufr_hd(bufr_file, path_out=None, VERSION=2):
    ...
    region = subprocess.check_output(
        ['pybufrkit','query','001001',bufr_file]).decode().split('\n')[-2]
    number = subprocess.check_output(
        ['pybufrkit','query','001002',bufr_file]).decode().split('\n')[-2]

    wmo_code = region + number

    if wmo_code == '87344':
        directory = 'COR/'
        print 'Reading BUFR',bufr_file
        print 'The station is',wmo_code,': Cordoba Aero'
    elif wmo_code == '87155':
        directory = 'SIS/'
        print 'Reading BUFR',bufr_file
        print 'The station is',wmo_code,': Resistencia Aero'
    elif wmo_code == '87244':
        directory = 'VMRS/'
        print 'Reading BUFR',bufr_file
        print 'The station is',wmo_code,': Villa Maria del Rio Seco'
    elif wmo_code == '87418':
        directory = 'MDZ/'
        print 'Reading BUFR',bufr_file
        print 'The station is',wmo_code,': Mendoza Aero'
    elif wmo_code == '87576':
        print 'The station is',wmo_code,': Ezeiza Aero, do not process'
        print
        return
    else:
        print 'Do not care about station',wmo_code
        print
        return

    # Now we parse the date and time of the baloon launching
    year = subprocess.check_output(
        ['pybufrkit','query','004001',bufr_file]).decode().split('\n')[-2]
    month = subprocess.check_output(
        ['pybufrkit','query','004002',bufr_file]).decode().split('\n')[-2]
    day = subprocess.check_output(
        ['pybufrkit','query','004003',bufr_file]).decode().split('\n')[-2]
    hour = subprocess.check_output(
        ['pybufrkit','query','004004',bufr_file]).decode().split('\n')[-2]
    minute = subprocess.check_output(
        ['pybufrkit','query','004005',bufr_file]).decode().split('\n')[-2]
    second = subprocess.check_output(
        ['pybufrkit','query','004006',bufr_file]).decode().split('\n')[-2]
    ...
    time_of_launch = datetime(int(year),
                              ...
                              int(minute),
                              int(second))
    print 'The time of launching was:',time_of_launch
    ...
    if VERSION == 1:
        ...
        year = subprocess.check_output(
            ['pybufrkit','info',bufr_file]) | grep year', shell=True).decode().split('\n')[-2].split(' ')[-1]
        month = subprocess.check_output(
    ...
    print 'The datetime of the file name will be:',time_of_sounding

    file_name = time_of_sounding.strftime('%y%m%d_%H')+'_'+wmo_code+'.lst'
    # print(file_name)
    # Here we can add the sounding site to the path
    file_name_path = path_out+directory+file_name

    if os.path.exists(file_name_path):
        print 'Already did',file_name_path
        print
        return
    ...
    print 'Starting to write in:',file_name_path
    with open(file_name_path, 'w') as fid:
        for p, h, t, td, dv, vv in zip(pressure, height, temp, temp_dew, dir_v, vel_v):
            fid.write('\t%6.1f\t%4d\t%6.2f\t%6.2f\t%3d\t%7.4f\n') % (p,h,t,td,dv,vv)
...
if not os.path.exists(path_out+'/processed'):
    os.makedirs(path_out+'/processed')

bufr_list = glob.iglob(path_in+'radiosondeos_*')

for bufr_file in bufr_list:
    convert_bufr_hd(bufr_file, path_out)
    os.rename(bufr_file, path_out+'processed/'+bufr_file.split("/")[-1])  # to move bufr already processed
