...
imageTypes = ['large_dbz_vel_ppi',
              'large_dbz_vel_rhi',
              'self_consistency',
              'self_consistency_scatter',
...
prodNames_raw = ['ppi',
                 ...
                 'z_kdp',
                 'z_kdp_cs']
prodNames = ['ppi_qc_level0_refl_vel',
             'rhi_qc_refl_vel',
             'self_consistency_z_zdr_vs_kdp',
             'self_consistency_scatter_z_zdr_vs_kdp',
             'self_consistency_scatter_kdp_vs_zdr_with_cs']
        ...
        for ftpFileName in ftpFileList:
            ...
            if (int(fileDateTimeStr) < int(startDateTimeStr)):
                ...
                if (localFileName not in localFileList):
                    ...
                    if imageTypes[idx] == 'self_consistency' or imageTypes[idx] == 'self_consistency_scatter':
                        (datePlus,time,junk1,junk2) = ftpFileNamePrefix.split('_')
                    elif imageTypes[idx] == 'self_consistency_scatter_cs':
                        (datePlus,time,junk1,junk2,junk3) = ftpFileNamePrefix.split('_')
                    else:
