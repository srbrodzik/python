...
from QtVariant import QtCore as core
...
from RadialDisplay import RadialDisplay,MyFrame,MplCanvas
...
class MainWindow(gui.QMainWindow):
    ...
    def __init__(self, parent=None, config_file=None, data_dir=None, colormap_dir=None):
        ...
        ## nc file dir
        self.versionStr = "PySolo 0.4"
        self.data_dir = data_dir
        if self.data_dir is None:
            self.data_dir = self.cfg.config['ncfile-dir']
...
#        self.createFieldSelection(['Default', ])
    ...
    def getDefaultFile(self,path):
        ...
        if files == []:
            ...
            Please edit your configuration file ('~/pysolo_config.xml') or
    ...
    def drawSweeps(self, sweeps,var_list):
        """ draw a set of sweeps - stubbed to draw a single sweep """
        ##TODO - draw multiple sweeps
        try:
            self.mpl.drawSweep(sweeps[0],var_list[0], 1.0)
        except RuntimeError, e:
            print e
            sys.exit(1)
    ...
    def keyPressEvent(self, event):
        ...
        if key == 46: # '.', toggle between last and current fields
            if self.last_var != None:
                ...
                self.drawSweeps(self.sweeps,[self.selected_var])
                
    def fileOpen(self):
        """ callback for File->Open menu action """
        result = gui.QFileDialog.getOpenFileName(self, "Open Data File", self.dir_ncfile, "netCDF data files (*.nc)")
...
def parse_arg():
    ...
    parser = argparse.ArgumentParser(description='PySolo v1.0')
...
def main(argv):
    ...
    app.setApplicationName("PySolo v0.1")
...
def start():
    """ entry point called by pysolo script """
    main(sys.argv)
