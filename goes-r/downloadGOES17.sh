...
echo "GOES17 ABI data downloader"
...
# and
# http://edc.occ-data.org/goes17/getdata/
...
for PRODUCT in $PRODUCTS; do
	...
	for DAY in $DAYS; do
		...
		aws s3 --no-sign-request ls --recursive noaa-goes17/ABI-$PRODUCT/$YEAR/$DAY/$HOUR/ | awk '{print $3";"$4}' >> FullList.txt
...
do
	...
	if [ $LOCALSIZE -ne $SIZE ]; then
	    ...
	    aws s3 --no-sign-request cp s3://noaa-goes17/$FULLNAME ./
	else
