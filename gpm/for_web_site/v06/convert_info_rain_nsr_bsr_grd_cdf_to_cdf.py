...
Extracts stratiform rainrate sum output from allStorms_xxx_v11x_xxx.pro saved in 
 .../classify/class_data/stats_class_v11x/NearSurfRain/MM/infoRainNSR_Stra*zip
...
for imonth in months:
    ...
    for iyear in years:
        ...
        for file in glob.glob('infoRainNSR_Stra_EchoCores_*_'+iyear+'_*.zip'):
            ...
            crr_var_id.long_name = 'broad stratiform core rainrate sum based on GPM 2Ku /NS/SLV/precipRateNearSurface'
            ...
            cct_var_id.long_name = 'number of broad stratiform core pixels with GPM 2Ku /NS/SLV/precipRateNearSurface greater than zero'

            srr_var_id = id.createVariable('bsr_storm_rain_rate_sum','f4',('time','latitude','longitude',),
                                           fill_value=missing_flt,zlib=True,complevel=5)
            srr_var_id.units = 'mm/hr'
            srr_var_id.long_name = 'broad stratiform storm rainrate sum based on GPM 2Ku /NS/SLV/precipRateNearSurface'

            sct_var_id = id.createVariable('bsr_storm_rain_rate_count','i4',('time','latitude','longitude',),
                                           fill_value=missing_int,zlib=True,complevel=5)
            sct_var_id.units = 'none'
            sct_var_id.long_name = 'number of broad stratiform storm pixels with GPM 2Ku /NS/SLV/precipRateNearSurface greater than zero'

            if verbose:
                print('Done with vars')
            
            # define global variables
            id.Title = 'broad stratiform monthly rain rate sums based on GPM 2Ku /NS/SLV/precipRateNearSurface and /NS/CSF/typePrecip'
            if thresLevel == 'str':
