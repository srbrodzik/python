...
# OLYMPEX cfad for paper
#fname = 'gpm-ku_cfad_lynn_land_total.txt'
fname = 'gpm-ku_cfad_lynn_land_total_NEW_2014-18.txt'

#fname = 'gpm-ku_cfad_lynn_land_total_NEW.txt'

# OLYMPEX cfad for paper
title = 'GPMKu (Oly) Over Land Norm CFAD'

#title = 'GPMKu (VancIsl) Over Land Norm CFAD'
#title = 'GPMKu (NOre) Over Land Norm CFAD'
#title = 'GPMKu (SOre) Over Land Norm CFAD'
#title = 'GPMKu (NCa) Over Land Norm CFAD'
#title = 'GPMKu (CCa) Over Land Norm CFAD'
#title = 'GPMKu (SCa) Over Land Norm CFAD'
...
if apply_filter:
    if output_format == 'eps':
        outfile = 'gpm-ku_cfad_lynn_land_total_'+str(int(min_refl_for_plot))+'db_filtered.eps'
    elif output_format == 'pdf':
        outfile = 'gpm-ku_cfad_lynn_land_total_'+str(int(min_refl_for_plot))+'db_filtered.pdf'
    else:
        print 'Output file not eps or pdf . . . exiting.'
        sys.exit()
else:
    if output_format == 'eps':
        outfile = 'gpm-ku_cfad_lynn_land_total_'+str(int(min_refl_for_plot))+'db.eps'
    elif output_format == 'pdf':
        outfile = 'gpm-ku_cfad_lynn_land_total_'+str(int(min_refl_for_plot))+'db.pdf'
    else:
        print 'Output file not eps or pdf . . . exiting.'
        sys.exit()
