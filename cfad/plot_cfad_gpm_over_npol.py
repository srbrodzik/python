...
fname = 'gpm-ku-over-npol-'+sector+'_cfad_total.txt'

title = 'GPM-Ku Total Normalized CFAD - Over NPOL - '+sector

outfile = 'gpm-ku-over-npol-'+sector+'_cfad_total.png'
...
fid_smoothed = open(outdir+'/gpm-ku-over-npol-'+sector+'_cfad_total_smoothed.txt','w')
for ilevel in range(0,numSmoothedLevels):
    rawdata_smoothed[ilevel,:].tofile(fid_smoothed,sep="\t",format="%d")
    fid_smoothed.write('\n')
fid_smoothed.close()

#find mode at each height starting at min_level_for_plot and write to file
fid = open(indir+'/gpm_modes_'+sector+'.txt', 'w')
